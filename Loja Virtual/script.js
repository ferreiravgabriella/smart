...
function adicionarAoCarrinho(nome, imagem, valor, quantidade) {
    ...
    const confirmacao = window.alert('Produto adicionado ao carrinho.');
...
function transformarEmInput() {
    const inputCupom = document.createElement('input');
    inputCupom.type = 'text';
    inputCupom.className = 'cupom-input';
    inputCupom.placeholder = 'Digite o cupom';

    inputCupom.style.width = '150px';

    const botaoCupom = document.querySelector('.cupom');
    botaoCupom.parentNode.replaceChild(inputCupom, botaoCupom);

    inputCupom.addEventListener('keyup', function(event) {
        if (event.key === 'Enter') {
            console.log('Cupom digitado:', inputCupom.value);
        }
    });
